...
parser.add_argument('--note',
                    type=str,
                    default=None,
                    help="add note to saving path")
parser.add_argument('--notconcat',
                    type=int,
                    default=0)
# parser.add_argument('--offset', type=int, default=250)
parser.add_argument('--grid_vis', action="store_true", default=False)
...
render_dir += str(args.scale)+'x'
if args.notconcat and args.train:
    render_dir += 'nocat'
if args.note:
    render_dir += "_"+args.note
...
# _,_,zshape = grid.links.shape
# grid.links[:,:,:zshape//2] = -1
...
with torch.no_grad():
    ...
    for img_id in tqdm(range(0, n_images, img_eval_interval)):
        ...
        if args.ray_len:
            ...
            cv2.putText(im, f"minv={minv:.4f} meanv={meanv:.4f} maxv={maxv:.4f}", (10, 20),
                        0, 0.5, [255, 0, 0])
            im = torch.from_numpy(im).to(device=device)
        im.clamp_(0.0, 1.0)

        if not args.render_path and not args.depth:
