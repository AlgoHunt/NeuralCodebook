...
#         self.sh_mlp = []
#         self.sh_mlp.append(nn.Linear(sh_channels,num_channels))
#         self.sh_mlp.append(nn.LeakyReLU())
#         self.sh_mlp.append(nn.Linear(num_channels,num_channels))
#         self.sh_mlp.append(nn.LeakyReLU())
#         self.sh_mlp = nn.Sequential(*self.sh_mlp)
        
#         self.pos_mlp = []
#         self.pos_mlp.append(nn.Linear(pos_channels,num_channels))
#         self.pos_mlp.append(nn.LeakyReLU())
#         self.pos_mlp.append(nn.Linear(num_channels,num_channels))
#         self.pos_mlp.append(nn.LeakyReLU())
#         self.pos_mlp = nn.Sequential(*self.pos_mlp)
        
#         self.adain = AdaptiveInstanceNorm1d(num_channels, num_channels)
        
#         self.out_mlp = []
#         self.out_mlp.append(nn.Linear(num_channels,num_channels))
#         self.out_mlp.append(nn.LeakyReLU())
#         self.out_mlp.append(nn.Linear(num_channels,num_channels))
#         self.out_mlp.append(nn.LeakyReLU())
#         self.out_mlp.append(nn.Linear(num_channels,output_channels))
#         self.out_mlp = nn.Sequential(*self.out_mlp)
        
#         self.attend = nn.Softmax(dim=-1)
        
#     def forward(self, sh, density, posemb):
#         x = self.sh_mlp(torch.cat([sh,density],-1))
#         pos = self.pos_mlp(posemb)
#         x = self.adain(input=x,style=pos)
#         x = self.out_mlp(x)
#         return x
...
# def train_network(codebooknet, B_share,, target_type='sh'):
def train_network(codebooknet, n_iters, xyz_table, batch_size, lr, weight_decay, B_share):
    optimizer = torch.optim.Adam(codebooknet.parameters(),lr=lr, weight_decay=weight_decay)
    # scaler = amp.GradScaler()
    for i in tqdm(range(n_iters)):
        sample_point = torch.randint(low=0,high=xyz_table.shape[0],size=[batch_size])
        ...
        d_target_sampled = d_target[sample_point,:]
        sh_target_sampled = sh_target[sample_point,:]

        # with amp.autocast():
        sh,d = codebooknet(sh_sampled, d_sampled, xyz_sampled_fourier)
        sh = sh_sampled + sh
        d = d_sampled + d

        loss_d =  torch.mean((d_target_sampled - d)**2)
        loss_sh = (sh_target_sampled - sh) ** 2
        if args.use_importance_reweighting:
            importance = new_importance_map[sample_point,:] 
            loss_sh = loss_sh * importance 
        loss_sh =  torch.mean(loss_sh)
        loss = loss_sh +loss_d *2
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        if i % 100 ==0:
            print(f'step {i}/{n_iters} SH Loss:{loss_sh.item()}  DLoss:{loss_d.item()} lr:{lr}')
...
train_network(CodebookNetAll, n_iters=args.n_iters, xyz_table=xyz_table,
 batch_size=args.batch_size, lr=args.lr_density, 
 weight_decay=args.weight_decay_density, B_share=B_share)
