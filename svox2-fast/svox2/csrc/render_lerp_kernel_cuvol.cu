...
const int TRACE_RAY_CUDA_THREADS = 512;
...
//     // const TreeSpec& tree_graph, 
//     const int32_t *tree,
//     float*  xyz,
//     const half**  out, float*  cube_sz) {
//     const float fN = 2;
...
//         // const auto skip = tree.child.data_ptr<float>()[sub_ptr];
//         const auto skip = tree[sub_ptr];
...
// * For ray rendering
__device__ __inline__ void trace_ray_cuvol_nowarp(
        const PackedSparseGridSpec& __restrict__ grid,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t lane_id,
    ...
    if (ray.tmin > ray.tmax) {
        out[0] = (grid.background_nlayers == 0) ? opt.background_brightness : 0.f;
        out[1] = (grid.background_nlayers == 0) ? opt.background_brightness : 0.f;
        out[2] = (grid.background_nlayers == 0) ? opt.background_brightness : 0.f;
        if (out_log_transmit != nullptr) {
            *out_log_transmit = 0.f;
        }
        return;
    ...
    while (t <= ray.tmax) {
        ...
        if (sigma > opt.sigma_thresh) {
            int32_t neighbor[8];
            get_neighbor(grid.links, grid.stride_x, grid.size[2], grid.sh_data_dim, ray.l, neighbor);
            
            float rgb[3] = {0.f,0.f,0.f};
            #pragma unroll 27
            for (int lane_id = 0; lane_id < 27; ++lane_id){
                uint32_t lane_colorgrp_id = lane_id % grid.basis_dim;     // ?
                uint32_t lane_colorgrp = lane_id / grid.basis_dim;        // ?
                float lane_color = 0;
                if (opt.use_nearest){
                    lane_color = trilerp_cuvol_one_with_neighbor(
                                neighbor,
                                grid.sh_data,
                                grid.stride_x,
                                grid.size[2],
            ...
            for(int ii=0;ii<3;++ii){
              out[ii]  += weight * fmaxf(rgb[ii] + 0.5f, 0.f);  // Clamp to [+0, infty)
            }
    ...
    if (grid.background_nlayers == 0) {
        for(int ii=0;ii<3;++ii){
              out[ii]  += _EXP(log_transmit) * opt.background_brightness; // Clamp to [+0, infty)
            }
    }
...
// * For ray rendering
__device__ __inline__ void trace_ray_cuvol_tree_nowarp(
        const PackedSparseGridSpec& __restrict__ grid,
        const PackedTreeSpec& __restrict__ tree,
        // const int32_t *tree,
        SingleRaySpec& __restrict__ ray,
        const RenderOptions& __restrict__ opt,
        uint32_t lane_id,
    ...
    while (t <= ray.tmax) {
        ...
        float cube_sz;
        const half* tree_val;
        // query_single_from_root(tree, ray.pos, &tree_val, &cube_sz);
        query_single_from_root(tree.child, real_pos, &cube_sz, print_flag);
        float invdir[3];
        #pragma unroll
        for (int i = 0; i < 3; ++i) {
            invdir[i] = 1.f / (ray.dir[i] + 1e-9);
        }
        const float t_subcube = _dda_unit<float>(real_pos, invdir) /  cube_sz;
        const float delta_t = t_subcube * grid.size[0] + opt.step_size;
        // if (print_flag){
        //     printf("xyz:%f %f %f delta: %f sample_num: %d cube_sz: %f\n",real_pos[0],real_pos[1],real_pos[2], delta_t, sample_num, cube_sz);
        // }
        // const float delta_t = opt.step_size;
        ...
        if (sigma > opt.sigma_thresh) {
            ...
            if (lane_id == 0) {
                trilerp_backward_cuvol_one_density(
                        ...
                        grads.viewcount_helper,
                        grads.mask_out,
                        grid.stride_x,
                        grid.size[2],
                        ray.l, ray.pos, ray.view,
                        curr_grad_sigma);
            }
            if (_EXP(log_transmit) < opt.stop_thresh) {
                break;
...
__global__ void render_ray_kernel(
    ...
    ray_spec[ray_blk_id].set(rays.origins[ray_id].data(), rays.dirs[ray_id].data(),rays.views[ray_id]);
...
__global__ void render_ray_image_nowarp_kernel(
        PackedSparseGridSpec grid,
        PackedCameraSpec cam,
        RenderOptions opt,
        float* __restrict__ out,
    ...
    // const int ix = ray_id % cam.width;
    // const int iy = ray_id / cam.width;
    const int ray_id = ix * cam.height + iy;
    if (within_bounds_2d(iy, ix, cam.height, cam.width)==false){
        return;
    }
    float sphfunc_val[9];
...
__global__ void render_ray_image_tree_nowarp_kernel(
        PackedSparseGridSpec grid,
        // const int32_t *tree,
        PackedTreeSpec tree,
        PackedCameraSpec cam,
...
__global__ void render_background_kernel(
    ...
    SingleRaySpec ray_spec(rays.origins[ray_id].data(), rays.dirs[ray_id].data(), rays.views[ray_id]);
...
torch::Tensor volume_render_cuvol_image_nowarp(SparseGridSpec& grid, CameraSpec& cam, RenderOptions& opt) {
    ...
    torch::Tensor log_transmit;
    if (use_background) {
        log_transmit = torch::zeros({cam.height, cam.width}, options);
    }

    {
        ...
        // const int blocks = CUDA_N_BLOCKS_NEEDED(Q * WARP_SIZE, cuda_n_threads);
        dim3 threads(16, 32);
        int d1 = (cam.width + threads.x - 1) / threads.x;
        int d2 = (cam.height + threads.y - 1) / threads.y;
        // int d3 = cam.height*cam.width;
        dim3 blocks(d1, d2);

        device::render_ray_image_nowarp_kernel<<<blocks, threads>>>(
                grid,
                cam,
                opt,
                // Output
    ...
    if (use_background) {
        // printf("RENDER BG\n");
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q, TRACE_RAY_BG_CUDA_THREADS);
        device::render_ray_image_nowarp_kernel<<<blocks, TRACE_RAY_BG_CUDA_THREADS>>>(
                grid,
                cam,
                opt,
                log_transmit.data_ptr<float>(),
...
torch::Tensor volume_render_cuvol_image_tree_nowarp(SparseGridSpec& grid, 
        TreeSpec& tree, CameraSpec& cam, RenderOptions& opt){
...

    {
        ...
        device::render_ray_image_tree_nowarp_kernel<<<blocks, threads>>>(
                grid,
                // tree.child.data_ptr<int32_t>(),
                tree,
                cam,
                opt,
                // Output
    ...
    if (use_background) {
        // printf("RENDER BG\n");
        const int blocks = CUDA_N_BLOCKS_NEEDED(Q, TRACE_RAY_BG_CUDA_THREADS);
        device::render_ray_image_tree_nowarp_kernel<<<blocks, TRACE_RAY_BG_CUDA_THREADS>>>(
                grid,
                // tree.child.data_ptr<int32_t>(),
                tree,
                cam,
