...
template<class voxel_index_t>
__device__ __inline__ float get_neighbor(
        const int32_t* __restrict__ links,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
        int32_t* __restrict__ neighbor) {
    const int32_t* __restrict__ link_ptr = links + (offx * l[0] + offy * l[1] + l[2]);
#define GET_LINK(u) link_ptr[u]*stride
    neighbor[0] = GET_LINK(0);
    neighbor[1] = GET_LINK(1);
    neighbor[2] = GET_LINK(offy);
    neighbor[3] = GET_LINK(offy+1);
    neighbor[4] = GET_LINK(offx);
    neighbor[5] = GET_LINK(offx+1);
    neighbor[6] = GET_LINK(offy+offx);
    neighbor[7] = GET_LINK(offy+offx+1);
#undef GET_LINK
}

// trilerp with neighbor
template<class data_type_t, class voxel_index_t>
__device__ __inline__ float trilerp_cuvol_one_with_neighbor(
        const int32_t* __restrict__ neighbor,
        const data_type_t* __restrict__ data,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
        const float* __restrict__ pos,
        const int idx) {
#define MAYBE_READ_LINK(u) ((neighbor[u] >= 0) ? data[neighbor[u] + idx] : 0.f)
    const float ix0y0 = lerp(MAYBE_READ_LINK(0), MAYBE_READ_LINK(1), pos[2]);
    const float ix0y1 = lerp(MAYBE_READ_LINK(2), MAYBE_READ_LINK(3), pos[2]);
    const float ix0 = lerp(ix0y0, ix0y1, pos[1]);
    const float ix1y0 = lerp(MAYBE_READ_LINK(4), MAYBE_READ_LINK(5), pos[2]);
    const float ix1y1 = lerp(MAYBE_READ_LINK(6), MAYBE_READ_LINK(7), pos[2]);
    const float ix1 = lerp(ix1y0, ix1y1, pos[1]);
    return lerp(ix0, ix1, pos[0]);
#undef MAYBE_READ_LINK
}
...
// trilerp with links
template<class data_type_t, class voxel_index_t>
__device__ __inline__ float try_trilerp(
        const int32_t* __restrict__ links,
        const data_type_t* __restrict__ data,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
...
__device__ __inline__ __half lerp_half(const __half a, const __half b, const __half w) {
    return __hfma(w,__hadd(b, __hneg(a)), a);
}

// trilerp with links
template<class data_type_t, class voxel_index_t>
__device__ __inline__ float trilerp_cuvol_one_half(
        const int32_t* __restrict__ links,
        const data_type_t* __restrict__ data,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
        ...
        const int idx) {
    const int32_t* __restrict__ link_ptr = links + (offx * l[0] + offy * l[1] + l[2]);
    const __half pos_2 =  __float2half(pos[2]);
    const __half pos_1 =  __float2half(pos[1]);
    const __half pos_0 =  __float2half(pos[0]);

#define MAYBE_READ_LINK(u) ((link_ptr[u] >= 0) ? __float2half(data[link_ptr[u] * stride + idx]) : __float2half(0.f))
    const half ix0y0 = lerp_half(MAYBE_READ_LINK(0), MAYBE_READ_LINK(1), pos_2);
    const half ix0y1 = lerp_half(MAYBE_READ_LINK(offy), MAYBE_READ_LINK(offy + 1), pos_2);
    const half ix0 = lerp_half(ix0y0, ix0y1, pos_1);
    const half ix1y0 = lerp_half(MAYBE_READ_LINK(offx), MAYBE_READ_LINK(offx + 1), pos_2);
    const half ix1y1 = lerp_half(MAYBE_READ_LINK(offy + offx), MAYBE_READ_LINK(offy + offx + 1), pos_2);
    const half ix1 = lerp_half(ix1y0, ix1y1, pos_1);
    const half final = lerp_half(ix0, ix1, pos_0);

    // const half ix0y0 = lerp_half(MAYBE_READ_LINK(0), MAYBE_READ_LINK(0), pos_2);
    // const half ix0y1 = lerp_half(MAYBE_READ_LINK(0), MAYBE_READ_LINK(0), pos_2);
    // const half ix0 = lerp_half(ix0y0, ix0y1, pos_1);
    // const half ix1y0 = lerp_half(MAYBE_READ_LINK(0), MAYBE_READ_LINK(0), pos_2);
    // const half ix1y1 = lerp_half(MAYBE_READ_LINK(0), MAYBE_READ_LINK(0), pos_2);
    // const half ix1 = lerp_half(ix1y0, ix1y1, pos_1);
    // const half final = lerp_half(ix0, ix1, pos_0);
    // return 0.f;
    return __half2float(final);
    // return __half2float(ix0);
#undef MAYBE_READ_LINK
}


template<class data_type_t, class voxel_index_t>
__device__ __inline__ float trilerp_cuvol_one_no_lerp(
        const int32_t* __restrict__ links,
        const data_type_t* __restrict__ data,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
...
#define MAYBE_READ_LINK(u) ((link_ptr[u] >= 0) ? data[link_ptr[u] * stride + idx] : 0.f)
    const float num0 = MAYBE_READ_LINK(0);
    const float num1 = MAYBE_READ_LINK(1);
    const float num2 = MAYBE_READ_LINK(offy);
    const float num3 = MAYBE_READ_LINK(offy + 1);
    const float num4 = MAYBE_READ_LINK(offx);
    const float num5 = MAYBE_READ_LINK(offx + 1);
    const float num6 = MAYBE_READ_LINK(offy + offx);
    const float num7 = MAYBE_READ_LINK(offy + offx + 1);
    ...
    // num6 = num6 + 1;
    // num7 = num7 + 1;
    // float ret = (num0+num1+num2+num3+num4+num5+num6+num7);
    float ret = (num0+num0+num0+num0+num0+num0+num0+num0);
    ret *= 0.125;
    return ret;
#undef MAYBE_READ_LINK
}
...
template<class data_type_t, class voxel_index_t>
__device__ __inline__ float nn_one(
        const int32_t* __restrict__ links,
        const data_type_t* __restrict__ data,
        int offx, int offy, size_t stride,
        const voxel_index_t* __restrict__ l,
...
__device__ __inline__ void trilerp_backward_cuvol_one_density(
        const int32_t* __restrict__ links,
        data_type_t* __restrict__ grad_data_out,
        int* __restrict__ viewcount_helper, 
        bool* __restrict__ mask_out,
        int offx, int offy,
        const voxel_index_t* __restrict__ l,
        const float* __restrict__ pos,
...
#define MAYBE_ADD_LINK_DEN(u, val) if (link_ptr[u] >= 0) { \
              atomicAdd(&grad_data_out[link_ptr[u]], val); \
              viewcount_helper[link_ptr[u]] |= 1 << view; \
              if (mask_out != nullptr) \
                  mask_out[link_ptr[u]] = true; \
        }
    MAYBE_ADD_LINK_DEN(0, ay * az * xo);
