...
__global__ void render_ray_image_kernel(
        ...
        RenderOptions opt,
        float* __restrict__ out) {
    CUDA_GET_THREAD_ID(tid, cam.height * cam.width);

    const int ix = tid % cam.width;
    const int iy = tid / cam.width;

    float dir[] = {0.f,0.f,0.f};
    float origin[]={0.f,0.f,0.f};
    BasicSingleRaySpec ray_spec(origin, dir);
...
torch::Tensor volume_render_svox1_image(SparseGridSpec& grid, CameraSpec& cam, RenderOptions& opt){
    DEVICE_GUARD(grid.sh_data);
    TORCH_CHECK(grid.basis_type == BASIS_TYPE_SH); // Only supporting SH for now
    grid.check();
    cam.check();
