...
class RenderOptions:
    ...
    def _to_cpp(self, randomize: bool = False):
        ...
        opt.use_nearest = self.use_nearest
...
class Rays:
    ...
    def _to_cpp(self):
        ...
        spec.dirs = self.dirs
        spec.views = self.views
        return spec

    def __getitem__(self, key):
        return Rays(self.origins[key], self.dirs[key], self.views[key])

    @property
    def is_cuda(self) -> bool:
        return self.origins.is_cuda and self.dirs.is_cuda and self.views.is_cuda
...
class Camera:
    ...
    def gen_rays(self) -> Rays:
        ...
        views = torch.ones_like(dirs)[:,0].int()
        if self.ndc_coeffs[0] > 0.0:
            origins, dirs = utils.convert_to_ndc(
                    ...
                    self.ndc_coeffs)
            dirs /= torch.norm(dirs, dim=-1, keepdim=True)
        return Rays(origins, dirs, views)
...
class _VolumeRenderFunction(autograd.Function):
    ...
    def backward(ctx, grad_out):
        ...
        # hack something, delete some grad
        cu_fn(
            ctx.grid,
            ctx.rays,
            ctx.opt,
...
class SparseGrid(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.view_count_thres = view_count_thres
        ...
        self.viewcount_helper = torch.zeros_like(self.density_data, dtype=torch.int, device=device)
        ...
        self.bit_all = torch.Tensor([2**i for i in range(30)]).cuda().type(torch.int32)
    ...
    def _volume_render_importance(self, rays: Rays, 
                            return_raylen: bool=False):
        """
        trilerp return importance
        """
        with torch.no_grad():
            # logger.info("using split render")
            origins = self.world2grid(rays.origins)
            dirs = rays.dirs / torch.norm(rays.dirs, dim=-1, keepdim=True)
            viewdirs = dirs
            B = dirs.size(0)
            ...
            pre_filter_mask = t <= tmax
            good_indices = good_indices[pre_filter_mask]
            origins = origins[pre_filter_mask]
            dirs = dirs[pre_filter_mask]

            #  invdirs = invdirs[mask]
            del invdirs
            t = t[pre_filter_mask]
            sh_mult = sh_mult[pre_filter_mask]
            tmax = tmax[pre_filter_mask]
            
            pre_filter_B = len(t)
            # logger.info(f"tmax shape: {tmax.shape}")

            ray_len = ((tmax - t )/self.opt.step_size).long()
            max_ray_len = ray_len.max()
            # sample_matrix = torch.zeros((pre_filter_B,max_ray_len), device = ray_len.device)
            sample_step_matrix = torch.arange(max_ray_len, device=ray_len.device).repeat([len(ray_len),1]) * self.opt.step_size
            sample_t_matrix = sample_step_matrix  + t.unsqueeze(-1)
            sample_mask = sample_t_matrix < tmax.unsqueeze(-1)
        
            pos = origins.reshape(pre_filter_B,1,3) + (sample_t_matrix.reshape(pre_filter_B,max_ray_len,1) * dirs.reshape(pre_filter_B,1,3))
            #pos.reshape(-1,3)
            pos = pos.clamp_min_(0.0)
            ...
            pos_sample = pos[sample_mask,:]
            
        sigma, rgb = self.sample(pos_sample, grid_coords=True,want_colors=False)
        
     
            
        with torch.no_grad():
            sigma_matrix = torch.zeros(pre_filter_B,max_ray_len,1, device=pos.device)
            sigma_matrix[sample_mask,:] = sigma
        
            log_att_matrix =  (
                    -self.opt.step_size
                    * torch.relu(sigma_matrix)
                    * delta_scale[good_indices].reshape(pre_filter_B,1,1)
                )

            # pre_filted stands for log_light_intensity before filted, 
            # store the light intensity in current sample point(after log), which is a monotone decreasing array 
            # shape is  batchsize *  ray_len * 1 
            log_light_intensity_pre_filted = log_att_matrix.cumsum(1).roll(1)  
            log_light_intensity_pre_filted[:,0,:] = 0   # set first element to zero

            # this log_light_intensity means light intensity in the ray end a.k.a background
            log_light_intensity[good_indices] = log_light_intensity_pre_filted[:,-1,0]
            
            #log_light_intensity_pre_filted 为 batchsize *  ray_len * 1 

            weight = torch.exp(log_light_intensity_pre_filted) * (1-torch.exp(log_att_matrix))  

            importance = weight[sample_mask,:]

            # for the VideoNeRF project, calculate fixed part here.
            fixed_part = torch.exp(log_light_intensity_pre_filted)[sample_mask,:] 

        # import pdb;pdb.set_trace()
       
        # sh_distill_loss = sh_distill_loss * 10

    
        return out_rgb , importance, sigma, fixed_part

    def init_viewcount_helper(self):
        self.viewcount_helper = torch.zeros_like(self.density_data, dtype=torch.int)
    ...
    def volume_render_fused(
        ...
        if not self.lazy_grad and init_viewcount_helper:
        #     self.viewcount_helper = torch.zeros_like(grad_density, dtype=torch.int)
            self.init_viewcount_helper()
        grad_holder = _C.GridOutputGrads()
        grad_holder.grad_density_out = grad_density
        grad_holder.grad_sh_out = grad_sh
        grad_holder.viewcount_helper = self.viewcount_helper
    ...
    def volume_render_image_tree(
        self, camera: Camera, tree_spec, use_kernel: bool = True, randomize: bool = False,
        batch_size : int = 5000,
        return_raylen: bool=False,
        nowarp=False,
    ):
        ...
        :return: (H, W, 3), predicted RGB image
        """
        imrend_fn_name = f"volume_render_{self.opt.backend}_image_tree"
        if nowarp:
            imrend_fn_name += "_nowarp"
        # import ipdb;ipdb.set_trace()
        if self.basis_type != BASIS_TYPE_MLP and imrend_fn_name in _C.__dict__ and not torch.is_grad_enabled() and not return_raylen:
    ...
    def volume_render_image_cpp(
        self, grid_cpp,opt_cpp, camera: Camera, use_kernel: bool = True, randomize: bool = False,
        batch_size : int = 5000,
        return_raylen: bool=False,
        nowarp=False,
    ):
    ...
    def save(self, path: str, compress: bool = False, save_rms=False):
        """
        Save to a path
        """
        ...
        if self.density_rms is not None and save_rms:
            data["density_rms"] =self.density_rms.data.cpu().numpy().astype(np.float32)
            data["sh_rms"] = self.sh_rms.data.cpu().numpy().astype(np.float32),
        if self.basis_type == BASIS_TYPE_3D_TEXTURE:
    ...
    def load(cls, path: str, device: Union[torch.device, str] = "cpu"):
        ...
        grid.viewcount_helper = torch.zeros_like(density_data, dtype=torch.int, device=device)
        ...
        # load rms state for training more smooth
        if 'density_rms' in z:
            grid.density_rms = torch.from_numpy(z.f.density_rms).to(device=device)
            print('---------------load density rms')
        if 'sh_rms' in z:
            grid.sh_rms = torch.from_numpy(z.f.sh_rms).to(device=device)
            print('---------------load sh rms')
